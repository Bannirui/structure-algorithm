    ...
    class Solution {
        public List<String> subdomainVisits(String[] cpdomains) {
            ...
            for (String cpdomain : cpdomains) {
                String[] cpdomainArr = cpdomain.split("\\s+");
                int counts = Integer.parseInt(cpdomainArr[0]);
                String domain = cpdomainArr[1];
                String[] subdomainArr = domain.split("\\.");
                int subdomainArrLen = subdomainArr.length;
                // 拼接子域名
                String curdomain = "";
                for (int i = subdomainArrLen - 1; i >= 0; i--) {
                    curdomain = subdomainArr[i] + (i == (subdomainArrLen - 1) ? "" : ".") + curdomain;
                    countMap.put(curdomain, countMap.getOrDefault(curdomain, 0) + counts);
                }
            }
            return countMap.entrySet().stream().map((entry) -> entry.getValue() + " " + entry.getKey()).collect(Collectors.toList());
        }
    }
